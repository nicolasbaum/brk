...
impl Vecs {
    pub(crate) fn forced_import(db: &Database, version: Version) -> Result<Self> {
        Ok(Self {
            ...
            ols_power_law_price: DayPrice::forced_import(db, "baseline_ols_power_law", version)?,
            ols_power_law_error: EagerVec::forced_import(
                db,
                "baseline_ols_power_law_error",
                version,
            )?,
            s2f_price: DayPrice::forced_import(db, "baseline_s2f", version)?,
            s2f_error: EagerVec::forced_import(db, "baseline_s2f_error", version)?,
            s2fx_price: DayPrice::forced_import(db, "baseline_s2fx", version)?,
